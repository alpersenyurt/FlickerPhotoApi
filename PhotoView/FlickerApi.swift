...
protocol FlickrPhotoSearchProtocol:class {
    ...
    func fetchPhotosForSearchText(searchText: String, onCompletion: (NSError?, [FlickrPhotoModel]?) -> Void) -> Void
...
//        static let TagsSearchFormat = "https://api.flickr.com/services/rest/?format=json&nojsoncallback=?&method=flickr.photos.search&tags=%@&extras=description,owner_name,url_s,url_l,url_o&api_key=%@&page=%d"
        ...
        static let TagsSearchFormat = "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=%@&tags=%@&format=json&nojsoncallback=1"
    ...
    func fetchPhotosForSearchText(searchText: String, onCompletion: (NSError?, [FlickrPhotoModel]?) -> Void) -> Void {
        ...
        let arguments: [CVarArgType] = [FlickrAPI.APIKey, escapedSearchText]
